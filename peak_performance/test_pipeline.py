...
def test_detect_raw_data():
    ...
    assert files == ['A1t1R1Part2_1_110_109.9_110.1.npy', 'A1t1R1Part2_2_111_109.9_110.1.npy', 'A1t1R1Part2_3_111_110.9_111.1.npy', 'A1t1R1Part2_4_112_110.9_111.1.npy', 'A1t1R1Part2_5_112_111.9_112.1.npy', 'A4_t4_1_Part2_Orn70_M4_m3.npy']
    pass
...
def test_prefiltering():
    ...
    assert list(df_summary_2.columns) == ['mean', 'sd', 'hdi_3%', 'hdi_97%', 'mcse_mean', 'mcse_sd', 'ess_bulk', 'ess_tail', 'r_hat', 'acquisition', 'experiment', 'precursor_mz', 'product_mz_start', 'product_mz_end', 'double_peak']
...
def test_single_peak_report_add_data_to_summary():
    ...
    assert len(df_summary.loc[:, "mean"].values) == 8
    assert list(df_summary.columns) == ['mean', 'sd', 'hdi_3%', 'hdi_97%', 'mcse_mean', 'mcse_sd', 'ess_bulk', 'ess_tail', 'r_hat', 'acquisition', 'experiment', 'precursor_mz', 'product_mz_start', 'product_mz_end', 'double_peak']
    assert list(df_summary.loc[:, "mean"]) == [5.577, 8.44, 25.989, 132.622, 0.516, 2181.193, 2762.597, 20.939]
    assert list(df_summary.loc[:, "acquisition"]) == len(df_summary.index) * ['A1t1R1']
    assert list(df_summary.loc[:, "experiment"]) == len(df_summary.index) * [4]
    assert list(df_summary.loc[:, "precursor_mz"]) == len(df_summary.index) * [118]
    assert list(df_summary.loc[:, "product_mz_start"]) == len(df_summary.index) * [71.9]
